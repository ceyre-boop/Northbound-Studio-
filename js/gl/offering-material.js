...
var PANEL_FRAG = [
  ...
  '  float fringe = 0.0022;',
  '  float blur = 0.05 * u_z * u_z;',
  '  float spread = fringe + blur;',
  '  vec2 nOff = n.xy * (refractPx / max(u_panelSize, vec2(1.0))) * tileSize;',
  '',
  '  vec2 base = u_tileMin + v_uv * tileSize + nOff;',
  '  vec2 uvR = clamp(base + dir * spread * tileSize, u_tileMin, u_tileMax);',
  '  vec2 uvG = clamp(base, u_tileMin, u_tileMax);',
  '  vec2 uvB = clamp(base - dir * spread * tileSize, u_tileMin, u_tileMax);',
  '',
  '  vec3 loop = vec3(',
  ...
  '  vec3 bloom = max(avg - 0.65, 0.0) * 1.8;',
  '',
  '  vec3 glass = mix(backdrop, loop + bloom, 0.72);',
  '',
  ...
  '     normal, scaled by the loop\'s own intensity so a quiet, far panel',
  '     does not catch as much light as the one being looked at. */',
  '  vec3 toLight = normalize(vec3(u_cursorPx - v_worldPos, 140.0));',
  '  float spec = pow(max(dot(n, toLight), 0.0), 24.0);',
  '  glass += vec3(0.9, 0.95, 1.0) * spec * 0.5 * u_intensity;',
  '',
...
var GRADIENT_FRAG = [
  ...
  '  vec3 bot = vec3(0.075, 0.095, 0.145);',
  '  vec3 col = mix(bot, top, v_uv.y);',
  '  float band = smoothstep(0.65, 0.0, abs(v_uv.y - 0.5 - 0.06 * sin(u_time * 0.05)));',
  '  col += vec3(0.05, 0.09, 0.12) * band * 0.4;',
