...
var PANEL_FRAG = [
  ...
  '',
  '  /* Fringe (colour separation) and recede-blur (softening) are the same',
  '     three atlas taps, but they are not the same LOOK, and sharing one',
  '     spread conflates them: pushing the taps apart as z rises reads as',
  '     prismatic separation, not as blur. So SPREAD (how far apart the three',
  '     taps sit, in pixels, converted through the panel\'s own on-screen size',
  '     exactly like the refraction offset above) and CHROMATICITY (how much',
  '     of that spread shows up as distinct colour vs. as grey softening) are',
  '     two different curves over z. Spread grows with z — a panel that is',
  '     SMALL on screen has more atlas texels per pixel, so it wants a wider',
  '     hand-rolled box filter, capped so it never resolves as three distinct',
  '     strokes. Chromaticity instead falls to zero well before |t| = 1.5, so',
  '     colour separation is a hero/near-band read only; past that the three',
  '     taps still move (for blur) but land on nearly the same colour. */',
  '  float chroma = 1.0 - smoothstep(0.0, 0.42, u_z);',
  '  float spreadPx = min(mix(1.2, 6.0, presence) + 15.0 * u_z * u_z, 15.0);',
  '  vec2 spreadUV = (spreadPx / max(u_panelSize, vec2(1.0))) * tileSize;',
  '  vec2 nOff = n.xy * (refractPx / max(u_panelSize, vec2(1.0))) * tileSize;',
  '',
  '  vec2 base = u_tileMin + v_uv * tileSize + nOff;',
  '  vec2 uvA = clamp(base + dir * spreadUV, u_tileMin, u_tileMax);',
  '  vec2 uvC = clamp(base, u_tileMin, u_tileMax);',
  '  vec2 uvB = clamp(base - dir * spreadUV, u_tileMin, u_tileMax);',
  '  vec3 texA = texture2D(u_atlas, uvA).rgb;',
  '  vec3 texC = texture2D(u_atlas, uvC).rgb;',
  '  vec3 texB = texture2D(u_atlas, uvB).rgb;',
  '',
  '  /* The SAME three samples read two ways: per-channel (chromatic) or',
  '     averaged across all three (a plain 1D box blur, no colour split at',
  '     all). Mixing between them by `chroma` is what lets the taps physically',
  '     keep spreading for blur while their colour contribution converges. */',
  '  vec3 loopChroma = vec3(texA.r, texC.g, texB.b);',
  '  vec3 loopBlur = (texA + texC + texB) / 3.0;',
  '  vec3 loop = mix(loopBlur, loopChroma, chroma);',
  '',
  '  /* Inner bloom: a cheap bright-pass of the same three samples (zero extra',
  '     taps), always read as the blurred/averaged colour so a loop stays',
  '     LEGIBLE as well as luminous — an earlier, hotter pass here (0.4',
  '     threshold, 2.6 gain) blew bright loops out into a formless glow.',
  '     Keeping the panel body out of true black is the ambient lift\'s job',
  '     below, not this one\'s. */',
  '  vec3 bloom = max(loopBlur - 0.58, 0.0) * 2.0;',
  '',
  '  vec3 glass = mix(backdrop, loop + bloom, 0.68);',
  '',
  '  /* Glass at rest still catches light even where the loop itself is dark —',
  '     a real pane is never truly black — so this ambient lift, tied to how',
  '     awake this loop is, carries the "panel body is not black" job on its',
  '     own rather than leaning on an over-hot bloom to do it. */',
  '  glass += vec3(0.06, 0.10, 0.13) * mix(0.4, 1.0, u_intensity);',
  '',
...
var PARTICLE_VERT = [
  ...
  '',
  '/* Same idea as drift-shaders.js Path D: per-particle flow strength so some',
  '   points barely move and some travel further, carried by a slow current',
  '   rather than each wandering independently. */',
  'void main() {',
  '  float flow = mix(0.5, 1.4, hash1(a_seed * 3.1 + 6.1));',
  '  vec2 pos = a_home;',
  '  pos.y += 0.030 * flow * sin(u_time * 0.11 + a_seed * 40.0);',
  '  pos.x += 0.022 * flow * cos(u_time * 0.08 + a_seed * 17.0);',
  '  vec2 clip = pos * 2.0 - 1.0;',
  '  clip.y = -clip.y;',
  '  gl_Position = vec4(clip, 0.0, 1.0);',
  '  float phase = hash1(a_seed * 3.1 + 12.0) * 6.283;',
  '  float twinkle = (0.5 + 0.5 * sin(a_seed * 53.0 + u_time * 0.8 + phase))',
  '                * (0.85 + 0.15 * sin(a_seed * 7.0 + u_time * 0.13));',
  '  v_lit = twinkle;',
  '  v_a = 0.26 + 0.55 * twinkle;',
  '  v_warm = step(0.855, hash1(a_seed * 3.1 + 50.0)); // roughly one in seven',
  '  v_soft = hash1(a_seed * 3.1 + 70.0);',
  '  float warmSize = mix(1.0, 1.22, v_warm);',
  '  gl_PointSize = u_pointScale * warmSize;',
  '}'
].join('\n');
...
var PARTICLE_FRAG = [
  ...
  '  vec3 cool = vec3(0.16, 0.22, 0.30);',
  '  vec3 warm = vec3(0.30, 0.22, 0.14);',
  '  vec3 dim = mix(cool, warm, v_warm);',
  '  vec3 litCool = vec3(0.33, 0.86, 0.80);', // aurora-adjacent
  '  vec3 litWarm = vec3(0.88, 0.63, 0.36);', // signal
  '  vec3 lit = mix(litCool, litWarm, v_warm);',
  '  vec3 col = mix(dim, lit, v_lit);',
  '  float a = core * v_a;',
  '  if (a < 0.004) discard;',
