 ...
 */
  ...
  /* Shared sprite shading for the two GPGPU render passes: a soft round dot,
  ...
  var SPRITE_FRAG_BODY = [
    ...
    '  vec3 dim = vec3(0.10, 0.16, 0.22);',
    '  vec3 lit = vec3(0.55, 0.92, 1.00);',
  ...
  var A_UPDATE_VERT = [
    ...
    '  float n = vnoise(pos.xy * 0.6 + u_time * 0.03 + a_seed * 4.0);',
    '  vec3 curl = vec3(n - 0.5, vnoise(pos.yz * 0.6 - u_time * 0.02) - 0.5, 0.0) * 0.22;',
    '',
    '  vec2  toCursor = pos.xy - u_cursor;',
    '  float cd = dot(toCursor, toCursor);',
    '  vec2  push = toCursor / (cd + 0.05) * 0.0035;',
    '',
    '  vel += (toHome * 0.6 + curl - vec3(push, 0.0)) * u_dt;',
  ...
  var A_RENDER_VERT = [
    ...
    '  float twinkle = 0.55 + 0.45 * sin(a_seed * 37.0 + a_life * 6.0);',
    ...
    '  gl_PointSize = min(u_pointScale * u_dpr, u_maxPoint);',
    '}'
  ].join('\n');
  ...
  var C_VEL_FRAG = [
    ...
    '  vec3 toHome = home - pos;',
    '  float n = vnoise(pos.xy * 0.6 + u_time * 0.03 + seed * 4.0);',
    '  vec3 curl = vec3(n - 0.5, vnoise(pos.yz * 0.6 - u_time * 0.02) - 0.5, 0.0) * 0.22;',
    '  vec2 toCursor = pos.xy - u_cursor;',
    '  float cd = dot(toCursor, toCursor);',
    '  vec2 push = toCursor / (cd + 0.05) * 0.0035;',
  ...
  var C_RENDER_VERT = [
    ...
    '  float twinkle = 0.55 + 0.45 * sin(seed * 37.0 + life * 6.0);',
  ...
  var D_VERT = [
    ...
    '',
    '  gl_Position = u_proj * vec4(pos, 1.0);',
    '  float twinkle = 0.55 + 0.45 * sin(a_seed * 37.0 + u_time * 0.6);',
    '  float lit = step(0.86, hash(vec2(a_seed, 3.0)));',
    '  v_lit = mix(0.12, 1.0, lit) * twinkle;',
    '  v_a = mix(0.10, 0.85, lit) * mix(0.6, 1.0, depth);',
    '  gl_PointSize = min(u_pointScale * u_dpr * depth, u_maxPoint);',
    '}'
  ].join('\n');
